...
**Files Tab（文件标签页）**：项目的文件浏览与编辑视图——展示该项目根下文件系统可见的**全部**条目（不按 `.gitignore` 等忽略规则过滤）。打开条目时按类型分流：文本进编辑器；图片内嵌预览；其余只读占位并可以系统应用打开。同一时刻**至多打开一个条目**（点树即切换，无内层多文件 Tab）。正文在左、文件树在右。**不做**文件管理（无新建 / 重命名 / 删除 / 复制 / 移动）——那些仍走系统文件管理器或 **Terminal**。每项目**恒有一个**、常驻 Tab 栏**第二位**（紧接 **Git Tab** 之后、会话 Tab 之前）、不可关闭；它不是会话（无进程、无输出流），是 Tab 模型中的非会话 Tab 之一（与 **Git Tab** 同类）。
