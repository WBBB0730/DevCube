...
**Changelog（更新日志）**：一个正式版本对应的一段手写说明，逐条列出用户能感知到的变化。Beta 不单独写，共用同号正式版那段；某一版可以不写。
_Avoid_: Release Notes, 发版说明, 更新说明, 提交记录（指日志内容时）
...
> **开发者**：那 Beta 更新时看到的 **Changelog** 是另写的一份吗？
> **领域专家**：不是。**Changelog** 只按正式版本写，Beta 用同号正式版那段；我从 1.8.0 升到 1.10.0，弹窗里就是 1.10.0 和 1.9.0 两段，哪版没写就不出现。它是手写给用户看的，不是提交记录。
