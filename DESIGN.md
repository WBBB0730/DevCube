...
- **设置弹层**（应用「设置」、Git「仓库设置」、命令配置新建/编辑共用 `SettingsModal` 外壳）：盖在主窗口上的全屏级模态（非第二窗口）——外框圆角 10px（`rounded-dialog`，对齐 macOS 窗口系统圆角）；顶栏标题居中、**加粗**（`font-bold`）、`--fg-dialog-title`；无标题栏关闭钮（Esc / 点遮罩关）。默认底栏「确定」（主色；点即关闭）；命令配置底栏为「取消 / 保存」。应用设置另有左侧分类树（无搜索）、右内容；分类项之间留 1px 缝（`gap-px`）。栏目：关于（版本、**Release Edition**、更新状态、检查更新、仓库链接）、偏好（全平台常挂）：「主题」分段按钮——深色 / 浅色，默认深色，选中即生效；其下「默认终端」下拉仅 Windows 出现——Git Bash / PowerShell / cmd，未检测到的项置灰、系统集成（全平台，**一列平铺、行距 12px、无分区小标题**：**External Open** 系统入口各占一行，「文件打开方式」与它们**同级占一行**，其下 **图片 / PDF / 音频 / 视频** 四个类型子项各占一行（不缩进、行距 4px），**`Info` 紧贴类型名成一块、该块定宽 64px，按钮紧跟其后贴左并对齐成列**——名字与动作分踞行两端会把行拉空，聚在左侧才读成一组）。行 = 名字 + 说明 12px `--fg-muted` + 右侧按钮；**顶级行名 13px `--fg-primary`、子项名降到正文色**表从属。入口行说明是「在 X 中添加 / 安装 Y」句式；「文件打开方式」这一行**自身没有按钮**（动作在四个子项上），说明为「把 X 设为这些类型文件的默认打开程序」。**子项不列扩展名**（对用户没有决策价值），改在名字后跟一颗 `Info`（`size-3.5`，`--fg-disabled`，hover 转 `--fg-icon`），hover 才显示 `.png .jpg …` 全表。行右按钮：入口行动词「安装 / 移除」，**子项压成两字「设置」**（完整动作「把 X 文件的默认打开程序设置为本应用」与 Windows 的落差——只能注册候选再跳系统「默认应用」页——都挂按钮 hover）；**已是默认不摆按钮**，改 `Check` + 12px muted「默认」（与「设置」等宽，四行按钮列齐平；hover 补全「已是默认打开程序」。无反向动作，按不动的按钮是噪声）；进行中按钮内转圈 `LoaderCircle`、文字隐形占位保宽。**子项一律保持「类型名 + 写着动作的按钮」**：设为默认改的是系统级关联、且系统没有「取消默认」这个反向动作，按钮得自解释（压到两字是版面所限，动词不可省）——刻意不把四项做成分段条 / 开关组那类「点一下即生效」的选择控件（那套视觉承诺的是可来回切换，与这里单向且不可撤销的语义相悖，段上也没有动词）。状态实时探测；Codex 未检测到桌面端则不出现该行，「打开于」仍置灰；子项全不可用则「文件打开方式」整块不渲染（Linux 只剩这一块）、快捷键（只读）。**提示类信息不内联进设置界面**：失败统一走「操作失败」错误框（`DialogMask + DialogPanel` + 等宽正文 + 「知道了」，Git 同款；Esc 先收错误框再关设置）。
...
| Files 看图 / PDF：回适应窗口 / 放大 / 缩小                | CmdOrCtrl+0 / + / -（焦点在预览内；渲染层自持，不经主进程，但主进程的 electron-toolkit 窗口钩子须开 `zoom` 才放行 - 与 Shift+=，见 ADR-0019；「适应窗口」title 带 ⌘0） |
