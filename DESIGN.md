...
- **过渡**：凡 hover 变底色 / 变图标色的图标钮与 Tab `×` 一律带 `transition-colors`（Tailwind 默认 150ms）；展开箭头旋转 `transition-transform`；滚动渐隐遮罩不走过渡，透明度随滚动位置连续变化（见下）。**滚动渐隐遮罩**统一用 `main.css` 的通用样式——外框 `.scroll-fade-x` / `.scroll-fade-y`（需 `relative`）包住滚动区 `.scroll-fade-scroller`，两端各一条「面板底色 → 同色全透明」渐变盖在内容上、不响应指针；透明度由 CSS 滚动驱动动画驱动：离两端 16px 内随滚动渐隐，内容不需滚动时不出现，打开首帧即就位，不写 JS。按处设 `--scroll-fade-color`（所在面板底色）、`--scroll-fade-gutter`（常驻滚动条填其宽度，让遮罩不盖滚动条），必要时 `--scroll-fade-size`。**hover 出钮**一律 `opacity-0 group-hover:opacity-100` + `transition` 渐显且不改变布局：钮常驻占位（提交面板文件行的 `…` 钮），或绝对定位盖在行尾、被盖住的尾字同步 `transition-opacity` 渐隐（工作树弹层的行尾钮，分支名平时贴右缘）；左树配置行的两个固定槽是例外（运行中恒显、原地替换，显隐切换）。
...
- **分段按钮**（JetBrains New UI SegmentedButton 的等价物，`ui/segmented-control.tsx`）：高 32px（`h-8`）外容器 6px 圆角 + `--border-input` 描边 + `--bg-panel` 底 + 1px 内边距；内部各段 4px 圆角、**不描边**，选中态纯靠底色 `--selection-row`（同左树选中行），未选中段 hover 走 `--bg-row-hover`。JetBrains 为该控件单列了 `SegmentedButton.selectedButtonColor` / `selected*BorderColor` 四个键，**刻意不采用**——只服务一个控件的颜色不该在全局 token 里单占，且描边 + 浮起底色的画法在本工作区会与容器描边叠成双层边。语义走 Base UI **Radio** 而非 ToggleGroup——后者单选态下再点已选项会取消，落到「一个都没选」，不适合必选项。 **选用**：两到三个互斥选项用分段按钮，四个以上再用下拉框。
...
Lucide（随 shadcn）：`Play`(运行) · `RotateCw`(重新运行 / Git 刷新) · `Square`(停止) · `Eraser`(清空运行会话输出) · `MoreVertical`(更多⋮) · `Pencil`(编辑) · `Trash2`(删除) · `ChevronRight`(树展开 / 弹层箭头) · `FolderPlus`(添加项目 / Preview Window「添加为项目」) · `AppWindow`(项目菜单「在新窗口中打开」) · `FolderOpen`(打开文件夹 / Files Tab 标签「文件」 / Files「在文件夹中显示」) · `FileClock`(Files「最近打开文件」) · `ListTree`(Files「在文件树中显示」) · `Minus`(Files「隐藏文件树」) · `PanelRight`(Files「显示文件树」) · `PanelLeft`(Files PDF / PPT「缩略图」开关，开着时点亮，两者共用一个开关状态) · `SquareArrowOutUpRight`(Files「在其他应用中打开」) · `Info`(设置里行名后的详情 hover，如「文件打开方式」各类的完整扩展名) · `Eye`(Files Markdown / SVG「预览」，回编辑用既有 `Pencil`) · `Undo2`(Files gutter 弹窗「回滚该块」) · `Copy`(同弹窗「复制旧文本」 / Files 正文「复制图片」 / 树菜单「复制路径」) · `Files`(Files「复制文件 / 复制文件夹」，正文与树菜单同款) · `ChevronsUpDown`(Files 文件树「全部展开」) · `ChevronsDownUp`(Files 文件树「全部折叠」) · `ListFilter`(Files 文件树「按类型筛选」，非全选时点亮) · `CornerLeftUp`(Preview Window 树空白区菜单「上一级文件夹」) · `CornerRightDown`(Preview Window 目录菜单「进入此文件夹」) · `FolderSymlink`(Preview Window「转到项目」，根已登记时替换「添加为项目」) · `FilePlusCorner`(新建配置) · `ArrowUpDown`(项目排序) · `AArrowDown`/`AArrowUp`(名称升/降序) · `ClockArrowDown`/`ClockArrowUp`(时间升/降序) · `Plus`(新建终端) · `Terminal`(终端 Tab) · `Search`(左栏项目筛选 / Files 树顶筛选 / 终端搜索框)/`ChevronUp`/`ChevronDown`/`X`(筛选清空 / 终端搜索框 / 关闭 Tab)。运行三角以 `--run-glyph` 上色。

应用顶栏 / 设置：`Settings`(应用设置齿轮；Git 仓库设置仍用同图标) · 更新按钮用 `CircleArrowUp`（有则显示、无则整颗隐藏；未包装开发另常驻一颗同图标的绿色本地预览钮，`--run-glyph` 上色）。
...
- **应用顶栏**（自定义窗口顶栏，高 40px、`--bg-panel`，对齐 WebStorm）：隐藏系统标题栏；macOS 保留原生红绿灯并留出安全拖拽区；Windows/Linux 用系统窗口按钮叠层。**中间**标题与 `document.title` 同一套——有当前 **Project** 为「`{名} — DevCube`」，否则「DevCube」——**加粗**（`font-bold` / 700），颜色与设置弹层标题同为 `--fg-dialog-title`。**右侧**：更新按钮（有待处理更新才显示，否则整颗隐藏）+ 设置齿轮（`Settings`）；未包装开发在宿主动作钮之后、更新按钮之前再常驻一颗绿色「预览更新日志（本地）」钮（打包产物没有）。可拖拽区与按钮 `no-drag` 分区；顶栏开出的弹窗渲染在拖拽区之外。产品范围见 `docs/prd/in-app-update.md`、`docs/prd/changelog.md`。
...
- **更新按钮**：可自动更新形态仅在「已下载可安装」时出现；Windows Portable 外观相同，在已知有新版本时出现。点击一律先开**更新弹窗**，确认后才执行：可自动更新→重启安装（有运行中 **Run Session** 时再走全局退出确认），Portable / 未包装开发→打开对应 GitHub Release。检查中/下载中/失败时不显示。不可关闭或跳过（弹窗「取消」只关弹窗）。
- **更新弹窗**（顶栏更新按钮、「关于」页「立即更新」、开发版绿色预览钮共用）：小对话框外壳加宽到 480px。顶部两行——标题「{显示名} {新版本} 可用」14px `font-semibold` `--fg-primary`，副标题「当前版本 {当前版本}」12px muted；其下日志直接排在正文里（不套内框、不画分割线），限高 `min(360px, 50vh)` 内部滚动、可选字（滚动区伸进面板右内边距并常留 8px 滚动条位：滚动条贴面板右缘、与日期隔 8px，有无滚动条日期都与标题 / 按钮右缘对齐；上下各 16px `--bg-elevated` 滚动渐隐遮罩（通用 `.scroll-fade-y`，让出 8px 滚动条）），版本新→旧、段间 12px 留白：段头一行，版本号 14px `font-semibold` `--fg-primary` 居左、日期 12px muted 居右；段内 13px 正文色紧凑列表（`list-disc`，圆点 `--fg-disabled`，条目间 2px，不套 prose；链接 `--link` 走系统浏览器）。一段都没有时正文写「无更新日志」（正文样式，不置灰）。底栏「取消」+ 主按钮：可自动更新「重启并更新」，Portable / 开发「前往下载」。开发版绿色钮开的是同一个弹窗，内容读工作区 `CHANGELOG.md`，「前往下载」去 Releases 列表页。
- **设置弹层**（应用「设置」、Git「仓库设置」、命令配置新建/编辑共用 `SettingsModal` 外壳）：盖在主窗口上的全屏级模态（非第二窗口）——外框圆角 10px（`rounded-dialog`，对齐 macOS 窗口系统圆角）；顶栏标题居中、**加粗**（`font-bold`）、`--fg-dialog-title`；无标题栏关闭钮（Esc / 点遮罩关）。默认底栏「确定」（主色；点即关闭）；命令配置底栏为「取消 / 保存」。应用设置另有左侧分类树（无搜索）、右内容；分类项之间留 1px 缝（`gap-px`）。栏目：关于（版本、**Release Edition**、更新状态、检查更新 / 立即更新——后者开**更新弹窗**、Esc 先收弹窗再关设置、仓库链接）、偏好（全平台常挂）：「主题」分段按钮——深色 / 浅色，默认深色，选中即生效；其下「默认终端」下拉仅 Windows 出现——Git Bash / PowerShell / cmd，未检测到的项置灰、系统集成（全平台，**一列平铺、行距 12px、无分区小标题**：**External Open** 系统入口各占一行，「文件打开方式」与它们**同级占一行**，其下 **图片 / PDF / PPT / Excel / 音频 / 视频** 六个类型子项各占一行（不缩进、行距 4px），**`Info` 紧贴类型名成一块、该块定宽 64px，按钮紧跟其后贴左并对齐成列**——名字与动作分踞行两端会把行拉空，聚在左侧才读成一组）。行 = 名字 + 说明 12px `--fg-muted` + 右侧按钮；**顶级行名 13px `--fg-primary`、子项名降到正文色**表从属。入口行说明是「在 X 中添加 / 安装 Y」句式；「文件打开方式」这一行**自身没有按钮**（动作在各子项上），说明为「把 X 设为这些类型文件的默认打开程序」。**子项不列扩展名**（对用户没有决策价值），改在名字后跟一颗 `Info`（`size-3.5`，`--fg-disabled`，hover 转 `--fg-icon`），hover 才显示 `.png .jpg …` 全表。行右按钮：入口行动词「安装 / 移除」，**子项压成两字「设置」**（完整动作「把 X 文件的默认打开程序设置为本应用」与 Windows 的落差——只能注册候选再跳系统「默认应用」页——都挂按钮 hover）；**已是默认不摆按钮**，改 `Check` + 12px muted「默认」（与「设置」等宽，各行按钮列齐平；hover 补全「已是默认打开程序」。无反向动作，按不动的按钮是噪声）；进行中按钮内转圈 `LoaderCircle`、文字隐形占位保宽。**子项一律保持「类型名 + 写着动作的按钮」**：设为默认改的是系统级关联、且系统没有「取消默认」这个反向动作，按钮得自解释（压到两字是版面所限，动词不可省）——刻意不把各项做成分段条 / 开关组那类「点一下即生效」的选择控件（那套视觉承诺的是可来回切换，与这里单向且不可撤销的语义相悖，段上也没有动词）。状态实时探测；Codex 未检测到桌面端则不出现该行，「打开于」仍置灰；子项全不可用则「文件打开方式」整块不渲染（Linux 只剩这一块）、快捷键（只读）。**提示类信息不内联进设置界面**：失败统一走「操作失败」错误框（`DialogMask + DialogPanel` + 等宽正文 + 「知道了」，Git 同款；Esc 先收错误框再关设置）。
...
- **克隆对话框**（左树「+」菜单「从 Git 仓库克隆…」；不绑定任何项目，故挂在 App 根而非 Git Tab 内；产品范围见 `docs/prd/clone-project-from-git.md`）：走**小对话框族**外壳（`FormDialogShell`，440px，与 Git 对话框同款），两态原地切换。**填写态**字段依次为仓库地址（等宽，placeholder 给 https 示例；打开时剪贴板里是仓库地址则预填；浏览器网页链接会在提交时归一成可克隆地址）/ 位置（等宽输入 + `FolderOpen` 图标钮开系统选择器，默认上次添加项目的父目录）/ 目录名（由地址派生——口径同 git 自己的取名——手改后不再跟随）/ 路径（只读等宽 muted 预览；目标目录已存在且非空时其下补一行 `--destructive` 提示且「克隆」置灰）/「包含子模块」勾选框（默认关）。**字段一律只有标签、不挂说明文字**（也不挂 `InfoIcon`）——四个标签各自自明，堆说明只会让这个高频小弹窗变吵。**克隆态**标题为「正在克隆 + 等宽地址」，地址**单行截断**、完整值走 `title`（长地址换行会把弹窗顶高、抖动版面）；其下阶段名 + 百分比（右侧等宽数字）+ 进度条（h-1.5、轨道 `--bg-row-hover`、填充 `bg-primary`、`transition-[width]`；git 未报百分比的阶段只显示阶段名、条留空），此时**遮罩与 Esc 都不收口**（`FormDialogShell` 的 `dismissible={false}`），只认明确点「取消」——中止一个跑了几分钟的克隆不该是误触的代价。取消与成功都关窗；失败回填写态、底部就地显示 git 原始报错（`--destructive`、可选中），输入原样留着供改了重试。
- **退出确认**：凡会退出整个应用的路径，若仍有运行中的 **Run Session**，二次确认；**Terminal** 不计入。macOS 仅关窗不退出不触发。文案：「还有 N 个运行会话在运行」/「退出应用将结束这些会话。确定退出？」。
- **左树**：固定 280px，**项目间留白**。Project（可折叠）下**直接列出 Run Configuration**（无「我的配置」小标题；**行背景不缩进、仅内容缩进对齐**——状态点对齐文件夹图标列、名称对齐项目名，靠"补空占位列 + 居中点盒"实现）；探测脚本收进一个**临时弹出菜单**（**Base UI Popover**，即项目组件库；背景 `--bg-elevated`（浮起于列表之上；深色下与面板同值，浅色下为纯白）；点触发浮出、点外面 / Esc 关闭，**选中或运行菜单项即刻关闭**；菜单项与配置行同款样式）。触发行置于配置列表最下方，UI 标签为**「检测到的配置」**（**文案与配置状态点对齐**，**数字在箭头前**，**箭头在右**）。项目行 hover 显示「更多」⋮；**右键或「更多」菜单打开期间**项目行保持与 hover 相同的行底（及 ⋮ / 角标让位），因指针已移入菜单会丢掉 `:hover`；配置行同理（右键 /「更多」打开期间保持行底与按钮可见）；项目行右键与「更多」共用菜单项：**打开文件夹**（`FolderOpen`）/ **打开于**（`SquareArrowOutUpRight`，子菜单）/ **在新窗口中打开**（`AppWindow`，以项目根开一个 Preview Window，见「Preview Window」）/ **新建终端**（`Terminal`）/ **新建配置**（`FilePlusCorner`）/ **置顶** 或 **取消置顶**（`Pin` / `PinOff`，在「移除」之上）/ **移除项目**。已 **Pin** 时空闲在「更多」同槽显示小图钉（muted），hover / 拖拽时**原地换成** ⋮（不挤到旁边）。无行内单独图钉按钮。项目名右侧显示包管理器角标（**pnpm 作为默认不显示**；hover 出按钮时让位隐藏）。链接工作树项目的文件夹图标由 `Folder` 换成 `FolderGit2`（hover 说明「xx」的工作树与主工作树路径；主工作树本身不换）。
...
- **Files Tab**：**每项目常驻第二个、不可关闭**（无 `×`，紧接 Git 之后、会话之前）。`FolderOpen` 图标 16px + 「文件」（14px）；左右内边距对称（同 Git，因无 `×`）。文件树（不按 `.gitignore` 过滤；隐藏 WebStorm Ignored Files 默认项如 `.git` / `.DS_Store`）+ 单文件正文：文本用 CodeMirror 6（实验对照分支；**仅语法高亮与文本编辑**——无 lint / 补全；Monaco 对照见 `backup/files-tab-monaco`；行号栏按相对 HEAD 的行状态显示 VCS 条纹——增绿 / 改蓝、删除为骑在行界上的灰短条，色取配色方案的 VCS 行状态（`--git-line-*`，随主题翻）；条纹 4px 圆头细条、水平贴行号列右缘，连续同状态行连成一段（段端上下各收 2px，段间留出空隙）；条纹本体可点（命中带 = 行号列右缘 6px，与 hover 加宽后的条同宽）：hover 以 hunk 为单位整块联动——pointer、条纹 4→6px / 删除短条放大（100ms 过渡，圆头 9999px 胶囊语义随宽度自适应），点击（松开时）弹出该改动块小窗——横向与编辑器内容区对齐并铺满（弹窗内文字与代码逐列对齐）、上边贴块尾行；工具条（`Undo2` 回滚该块 / `Copy` 复制旧文本 / `ChevronUp`·`ChevronDown` 上一个·下一个改动，跳转先把目标块滚到编辑器上 40% 固定位）+ 基线旧行只读预览（同主题语法高亮，added 块只出工具条），Esc / 点外 / 编辑 / 手动滚动即关；Markdown / SVG / CSV 可在「编辑 ↔ 预览」两态切换（打开即预览态，新建的空文件落编辑态），Markdown 预览 react-markdown + GFM、prose 排版对齐工作区 token，SVG 预览走 data URL 的 `<img>`（脚本不执行），CSV 预览是与 Excel 同一张只读表格，见 `docs/prd/files-diff-gutter.md` / `files-markdown-preview.md`）；图片内嵌预览（内容区右上角 12px `--fg-info` 显示像素宽高 `宽 × 高`，SVG 预览同；Cmd/Ctrl+滚轮按光标缩放（触控板捏合同手势；手势中只动 CSS transform，停手 100ms 后按新尺寸重新栅格化——捏合过程略软、松手即清晰）；超大位图（>3200 万像素或单边 >16384）先出预览图、后台切瓦片金字塔后叠 OpenSeadragon 瓦片层，手感同普通图；邻图预载；换图时新图 120ms 内没就绪就盖上居中 `LoaderCircle` +「正在加载…」（不留错位的旧图与尺寸标注）；浏览器解不了的图改走超大位图同一条主进程出图的路，仍打不开则盖上「无法预览此图片」占位（与 PDF / PPT / 表格同一占位，多一颗「重试」）；滚轮或拖拽平移、方向键切上一个/下一个媒体（位图 / SVG / PDF / 音视频同一序列，按树里当前可见的顺序：跨目录、只进已展开的目录、尊重预览窗口的类型筛选，到头停下；看图与音视频四键都切、音视频即使焦点在播放器上也拦下，PDF 与 PPT ←/→ 直接切，↑/↓ 翻页、翻到第一页 / 最后一页再按也切）；**正文右键菜单**（任何已打开的条目；受控 ContextMenu + 鼠标点虚拟 anchor，同树菜单）：复制图片 `Copy`（仅看图 / SVG 预览态）/ 复制文件 `Files` / 在文件夹中显示 `FolderOpen` / 在其他应用中打开 `SquareArrowOutUpRight`；工具栏钮组最左四档「自画的 `1:1` 方框图标 · `GalleryVertical` 适应高度 · `GalleryHorizontal` 适应宽度 · `Scan` 适应窗口」一组并以竖线隔开，SVG 预览态同，与 PDF 共用同一组钮——1:1 是像素对像素；两条轴按下后该轴铺满视口，另一轴溢出则顶 / 左对齐、不溢出则居中；适应窗口是整图可见且小图不放大。**四档互斥点亮**（`--selection-row` 蓝底 + 正文色、hover 不再变色，同查找栏方形开关），滚轮缩出自由倍率则全灭；**预览区双击在两条轴之间切换**（以双击点为锚点），**Cmd/Ctrl+0** 回适应窗口、**Cmd/Ctrl +/-** 逐档缩放（应用菜单的视图块已摘掉整页缩放三项让位，见 ADR-0019）；**打开与换图时装得下视口的图落 1:1、装不下的落适应窗口**（两者相机同形，只是档位不同，小图因此天然不放大）；视口尺寸一变（拖窗口 / 文件树显隐）所处档位按新尺寸重算，自由倍率只夹紧边界，尺寸塌成 0（Tab 切走是 display:none）时一概不动）；可播音视频居中原生控件预览；PDF 内嵌预览（PDF.js viewer 组件：连续滚动、文字可选、内链可点；面包屑之前领头一颗 `PanelLeft` 缩略图开关（开着时点亮，激活态同四档钮；侧栏在正文左侧，钮也靠左）；工具栏钮组最左加「页码输入 `n / N` · 竖线 · 自画的 `1:1` 方框图标 · `GalleryVertical` 适应高度 · `GalleryHorizontal` 适应宽度 · `Scan` 适应窗口」一组并以竖线隔开（四档与看图共用同一组图标、文案与点亮规则，一一对应 PDF.js 的 `page-actual` / `page-height` / `page-width` / `page-fit`；1:1 是纸张实际尺寸而非像素对像素；**打开即适应窗口**，同看图开场；无加减档与百分比读数，倍率调节归滚轮与捏合；页码框聚焦不描亮边、只压深底色，同树顶筛选框，与其后钮以竖线隔开）；Cmd/Ctrl+滚轮与捏合按光标缩放、手感同看图；空白处拖拽或按住空格拖拽抓手平移（grab / grabbing 同看图），文字上拖拽仍是选字；**空白处双击**在两条轴之间切换（以双击点为锚点；文字上双击仍是选词），**Cmd/Ctrl+0** 回适应窗口、**Cmd/Ctrl +/-** 按视口中心逐档缩放（同看图）；滚动容器不留上下内边距（库按容器高算「适应高度」，留白会让整页看不全），页间留白走库的 `.page` margin；容器尺寸变化后预设档按新尺寸重算（尺寸为 0 时不动，否则页面会被缩没）；Cmd+F 出与编辑器同款查找栏、无正则开关，当前命中滚到视口正中（同编辑器）；容器 `--bg-deepest`、页面白底带阴影、命中黄 / 选中橙、链接 hover 20% `--link` 色块；**缩略图侧栏**在正文左侧、固定宽 168px 不可拖（`--bg-deepest` 垫底、右缘 1px `--separator`），每格 = 白底带阴影的小页（宽 128px、高按该页真实宽高比）+ 其下 12px `--fg-info` 页码，留白统一 8px（侧栏内边距 / 格子内边距 / 小图到页码 / 页码到底边 / 格子之间）；格子三态底色由深到亮：常态 `--bg-panel`（比侧栏底亮一档）、hover `--bg-row-hover`、当前页 `--selection-row` 圆角底并把页码转 `--fg-primary`（同树行）；点格跳页并把焦点还给正文，翻页跟随点亮并以最小距离滚入视口；**↑/↓ 始终上一页 / 下一页**（正文与侧栏内都如此，不看档位）；小图一次只画一张、离视口最近的先画、顺滚动方向预取两屏、画好的存成压缩图整个文档期间不扔（细节见 `files-pdf-preview.md`）；开关钮领在面包屑之前、开着时点亮（激活态同四档钮 / 查找栏方形开关），默认显示、会话内保持不落盘、切换文件保持；列表虚拟滚动（@tanstack/react-virtual）只画视口内格子）；PPT 内嵌预览（pptx 及其放映版 / 模板 / 带宏版本；pptx-renderer 单页渲染、DevCube 自排页面列表，**外壳与 PDF 共用**——面包屑前缩略图开关、钮组最左「页码 · 竖线 · 四档」、Cmd/Ctrl+滚轮与捏合按光标缩放、空白处双击换轴、Cmd/Ctrl+0 / + / -、抓手与按住空格抓手、↑/↓ 翻页、Cmd+F 同款查找栏无正则开关；1:1 是幻灯片原尺寸；打开即适应窗口；页面保持原样白底、阴影同 PDF，页间 10px、窄于视口时水平居中；文字上 text 光标、链接由库给 pointer，抓手模式处处 grab；查找用 CSS Custom Highlight 只标命中的字，命中黄 / 当前橙同 PDF、当前命中滚到视口正中；缩略图侧栏与 PDF 同一套，格内是缩小画的整页；细节见 `files-pptx-preview.md` / ADR-0033）；Excel 内嵌预览（xlsx 及其模板 / 带宏版本与老 xls；react-xlsx 只读表格；表格保持原本浅色、四周 `--bg-deepest`；**底部工作表标签条**（h-8、`--bg-panel`、顶边 1px `--separator`；每张表一颗 h-6 圆角按钮、13px，当前表 `--selection-row` 底 + 正文色，其余 muted、hover `--bg-row-hover`；只有一张表时不出、隐藏的表不列）；无四档与页码——Cmd/Ctrl+滚轮与捏合按光标缩放、Cmd/Ctrl +/- 按视口中心逐档（手感同 PDF / PPT）、Cmd/Ctrl+0 回工作表自带倍率；方向键归表格移动选中格（不切文件），选中与 Cmd+C 复制用库的；Cmd+F 同款查找栏、无正则、只查当前工作表，命中格叠半透明黄、当前命中叠橙（同 PDF）并选中、滚入视口；加载中盖 muted「正在加载…」；超过 10 MB 占位「文件较大」+「在其他应用中打开」；细节见 `files-xlsx-preview.md` / ADR-0034）；其余占位并提供「在其他应用中打开」。工具栏：相对路径**可点面包屑**（左对齐；段间 `ChevronRight`、目录段 muted、文件名有工作区 Git 状态则用 `--git-status-*` 否则 `--fg-primary`；hover 只改文字色、无底；点目录/文件名 → 右侧树展开并滚到对应行；无未保存圆点）+ Markdown / SVG / CSV「预览 / 编辑」两态切换（`Eye`/`Pencil`，仅这三类显示，位于右侧钮区最左、与其后钮以 1×12px `--border-input` 竖线隔开；切入预览前自动保存）+「最近打开文件」(`FileClock`，下拉最近 10 个：文件名正文色 + 目录路径 muted，空则「暂无最近打开文件」；无正文时也显示；CmdOrCtrl+E 从任意位置切到 Files Tab 并弹出此下拉、预选第一个非当前文件，↑ / ↓ 选择、回车或空格确认，选中或 Esc 后焦点回正文) +「在文件树中显示」(`ListTree`，定位当前文件；若树已隐藏则先显示再滚到行) +「在文件夹中显示」(`FolderOpen`) +「在其他应用中打开」(`SquareArrowOutUpRight`) + 文件树隐藏时最右「显示文件树」(`PanelRight`，与左侧钮组之间 1×12px `--border-input` 竖线)；**双击编辑区标题栏**切换文件树显隐（面包屑与右侧钮组不触发），文案对齐 Git 菜单 / VS Code Reveal。**文件树行交互对齐左树、尺寸更紧凑**：树顶 header（同高 h-10）左侧常驻筛选（`Search` + 占位「筛选」，有内容时清空 `X`，样式同左栏项目筛选；冷索引首查时树区居中「正在扫描…」+ `LoaderCircle` 转圈，120ms 延迟出现防闪烁，同 diff 加载骨架）；焦点在树上时可打印字写入、Esc 清空；按相对路径包含收窄**同一棵树**（基于 ripgrep 文件名索引，ADR-0027；保留结构；目录命中带整支；无匹配「无匹配文件」；不过滤高亮）；右侧「全部展开」(`ChevronsUpDown`) /「全部折叠」(`ChevronsDownUp`) / 最右「隐藏文件树」(`Minus`)（过滤期间展开/折叠只作用于过滤后的树）；树顶栏之下、列表之上固定一行**当前文件夹**（h-8、`FolderOpen` `--fg-icon` + 文件夹名 `font-medium`，hover 全路径；图标**顶格**——它是全树的根，不占子级行那个层级/箭头占位；hover / 菜单打开时 `--bg-row-hover` 行底，无选中态；右键即空白区那份根菜单，文件铺满时也点得到；Preview Window 同此，换根后也靠它认出根在哪）；固定宽 280px；可整块隐藏（编辑区占满，不留窄条把手；可见性不持久化，重挂载默认展开）；**树行虚拟滚动**（@tanstack/react-virtual：按展开态拍平成行数组、行高固定 32、仅渲染视口内行——过滤命中或展开条目再多，渲染成本恒定；滚动定位按行下标而非 DOM 查找）；`h-8` / `rounded` / 13px / `transition-colors`；目录名默认 `text-foreground`、选中 `--fg-primary`；文件名/图标按工作区 Git 状态色（同提交面板 `FILE_STATUS_COLOR`，无状态时图标 `--fg-icon`、名正文色）；**文件图标按类型**（`filesTreeIconKind` 按扩展名归大类，只看名字）：一律取**裸物件**图标（内容是什么就画什么），不用「文件 + 角标」系列：图片 / SVG `Image` · 音频 `Music` · 视频 `Film` · md / txt / log `Type` · json `Braces` · csv / tsv / Excel `Table2` · sh / ps1 等 `Terminal` · zip / tar 等 `Package2` · 其余源码与配置 `Code`；PPT `Presentation`（讲台）· PDF 自绘 `SquarePdf`（外壳取 lucide `Square` 的圆角方框，同 `SquarePlay` 一族；框内居中一笔 Acrobat 风格卷纹——16 格单笔 stroke 路径缩放 0.8 居中，线宽经缩放约 1.4 比外框细一档）；未知二进制才用 `File`（其义正是「只知道是个文件」），选中压成 `--fg-primary`；选中行 `--selection-row`（hover 不变色），未选中 `hover:bg-row-hover`；行背景全宽、仅内容按层级缩进；图标/箭头 14px（`size-3.5`；文件夹/文件图标 `--fg-icon` 或状态色，箭头 `muted` 不同色）；目录始终 `Folder`（展开不换 `FolderOpen`，靠箭头旋转表示开合）。**编辑器内查找**（Cmd/Ctrl+F，焦点在编辑器时）：**编辑器顶部整宽查找栏**（h-9、`bg-panel`、底边 `--separator`，占位压下正文——对齐 WebStorm 编辑器查找栏形态，右上浮层版试过而改；控件样式仍同查找族）——`Search` + 输入 + 计数 x/y（封顶 999+，无结果「无结果」；坏正则原位红字）+ 大小写 / 全词 / 正则三颗方形图标开关（同内容搜索面板）+ `ChevronUp`/`ChevronDown` 导航（Enter / Shift+Enter，回绕循环、命中滚至视口中部并设为选区）+ `X`；Esc 关闭回焦编辑器。引擎复用 @codemirror/search 的 SearchQuery，**默认搜索面板退役**（其高亮与面板生命周期绑死，浮层形态由 `cm6-find` 自持同款高亮插件；searchKeymap 一并关闭，Alt-G 跳行 / Cmd-D 选下一个等默认键随之移除）；命中色即既有 `.cm-searchMatch` 主题（TEXT_SEARCH_RESULT）。不做替换。事件自动保存；无内层多文件 Tab。**树行 / 空白区右键菜单**（受控 ContextMenu + 鼠标点虚拟 anchor，同 Git 图谱菜单模式）：菜单项带图标（同左树项目菜单）、按四组排布：**新建**（新建文件 `FilePlus` / 新建文件夹 `FolderPlus`，全部目标；文件行作用于所在目录）→ **打开**（在文件夹中显示 `FolderOpen`，全部；在其他应用中打开 `SquareArrowOutUpRight`，仅文件；在终端中打开 `Terminal`，全部——文件开在所在目录、cwd 限项目内且不随壳持久化）→ **复制信息**（复制文件 / 复制文件夹 `Files`，全部；复制路径 `Copy`，全部；复制相对路径 `Copy`，非根）→ **改动自身**（重命名 目录 `FolderPen` / 文件 `FilePen`、删除 `Trash2`，非根；危险项垫底，同左树「移除项目」）。菜单打开期间目标行保持 hover 行底（同左树），新建与重命名走通用小对话框外壳（`ui/form-dialog`，同 Git 对话框族：WebStorm 式提示语 + 输入 + 底部按钮条（无分割线），Enter 提交防输入法合成、提交中锁死弹窗；磁盘冲突弹窗同壳；重命名预选主文件名），删除确认后移入系统回收站；新建文件成功即打开，重命名 / 删除联动当前打开文件与最近列表。复制 / 移动不做（CONTEXT.md 边界）。左栏 ProjectTree 不动。Git「打开文件」进入本 Tab。切走隐藏不卸载；每项目持久化上次打开路径与树展开（路径无效则静默空态）。布局为左正文 / 右文件树；未打开条目时空态文案「在右侧选择文件」。
- **Preview Window**（预览窗口，术语见 CONTEXT.md、范围见 `docs/prd/file-preview-window.md`）：DevCube 作为「打开方式」被带着文件拉起时的独立窗口——同一渲染入口按查询串挂壳：顶栏 = 居中标题（文件名；macOS 标题栏带代理图标）+ 右侧「添加为项目」`FolderPlus`（根已登记时换 `FolderSymlink`「转到项目」；同设置钮样式，排在更新 / 设置之前）+ 与主窗口**完全一致**的更新 / 设置钮，设置弹窗也是同一个（更新状态由主进程广播到全部窗口）；正文就是上面的 Files 面板（preview 宿主），无 Tab 栏 / 左树。差异只有：树顶栏（筛选框那一行）并入预览专属控件——右侧钮组最左「按类型筛选」`ListFilter`（下拉七项勾选 图片 / PDF / PPT / Excel / 音视频 / 文本 / 其他 + 分隔线 +「全部」；勾选项走 `DropdownMenuCheckboxItem`——左侧 14px 勾位、点击不关菜单；非全选时钮以 `--fg-primary` 点亮；目录恒显示、文件按类别过滤、与路径筛选取交集、换根保留、不落盘）；主窗口 Files Tab 没有这些）；树菜单无「在终端中打开」，「打开」组后多一组根导航：空白区 / 根出「上一级文件夹」(`CornerLeftUp`，到文件系统根置灰) 与「添加为项目」(`FolderPlus`；已登记时换 `FolderSymlink`「转到项目」，与顶栏那颗同源)，目录出「进入此文件夹」(`CornerRightDown`，根自身没有；与「上一级文件夹」的 `CornerLeftUp` 互为 180° 镜像，一进一出一眼可辨，也避开菜单里成片的文件夹图标)；树展开 / 最近 / 当前文件不落盘；Markdown / SVG / CSV 起手即预览态（同 Files Tab）。窗口默认 1000×680（最小 640×420），几何与主窗口分开记忆（同样只记到退出）；新开窗继承最近聚焦且还开着的预览窗口此刻的状态——它最大化新窗也最大化，否则逐窗错位 24px；Cmd/Ctrl+W 关窗（主进程拦），⌥⌘F 仍聚焦树顶筛选，⌘E 仍弹出「最近打开文件」。
...
- **选中态** 3px `--primary` 主色下描边、**hover** `--bg-row-hover`；每个 Tab 均保持单行且不参与收缩，过多时只允许横向溢出，滚动条隐藏且不出现纵向滚动。左侧无留白，右侧以固定占位保留 8px；两侧各有 16px 的 `--bg-panel` 滚动渐隐遮罩（通用 `.scroll-fade-x`，底边留 1px 不盖）：离对应边缘 16px 内随滚动渐隐、抵达边缘时消失，没有溢出时两侧都不出现；遮罩不响应指针。鼠标悬停栏内时普通滚轮直接横向滚动，无需按 Shift；触控板原生横向手势保持原方向。终端 Tab 的拖拽同样只沿横轴移动。每次激活 Tab（包括切换项目后恢复其激活 Tab）时，若目标不在横向视口内，则以最小距离自动滚入视口，并避开渐变遮罩。
...
| 切 Files Tab 并弹出「最近打开文件」                       | CmdOrCtrl+E（Win/Linux 焦点在终端时让给 shell 的行尾）                                                                                                                 |
| 上一 / 下一项目（左树当前排序+筛选可见序，循环）          | Alt+CmdOrCtrl+↑ / ↓                                                                                                                                                    |
| Git：查找 / 刷新                                          | CmdOrCtrl+F / CmdOrCtrl+R（仅 Git Tab）                                                                                                                                |
| Files 编辑器：查找                                        | CmdOrCtrl+F（焦点在编辑器时）                                                                                                                                          |
| Files 看图 / PDF / PPT：回适应窗口 / 放大 / 缩小          | CmdOrCtrl+0 / + / -（焦点在预览内；渲染层自持，不经主进程，但主进程的 electron-toolkit 窗口钩子须开 `zoom` 才放行 - 与 Shift+=，见 ADR-0019；「适应窗口」title 带 ⌘0） |

注册方式：主进程 `webContents.before-input-event` 匹配并 `preventDefault`，再 IPC 到渲染端执行（窗口聚焦时优先于页面/xterm/Chromium 默认；**不能**压过 macOS Mission Control 等系统级快捷键，也不使用 `globalShortcut` 抢其它 App）。CmdOrCtrl 按平台严格匹配：macOS 只认 ⌘、Windows / Linux 只认 Ctrl，⌃T / ⌃W / ⌃F 等留给 shell 与编辑器；Windows / Linux 焦点在终端时 Ctrl+E 让给 shell，其余照常拦截。文案规则与让位机制见 ADR-0013。
