...
- **设置弹层**（应用「设置」、Git「仓库设置」、命令配置新建/编辑共用 `SettingsModal` 外壳）：盖在主窗口上的全屏级模态（非第二窗口）——外框圆角 12px（`rounded-xl`）；顶栏标题居中、**加粗**（`font-bold`）、`--fg-dialog-title`；无标题栏关闭钮（Esc / 点遮罩关）。默认底栏「确定」（主色；点即关闭）；命令配置底栏为「取消 / 保存」。应用设置另有左侧分类树（无搜索）、右内容；分类项之间留 1px 缝（`gap-px`）。本轮栏目：关于（版本、**Release Edition**、更新状态、检查更新、仓库链接）、快捷键（只读）。有真实偏好项之前不挂「偏好」栏。
