...
各类扩展名表与 Files 面板的类型判定同源（`FILES_OPEN_WITH_EXTS`），保证设成默认的类型恒能内嵌预览；`svg` 归图片组，虽然它的打开分流是文本（打开即预览态）。音频与视频在设默认上分开（用户常只想接管其一），在 Files 树顶的类型筛选里仍并作「音视频」一类。
