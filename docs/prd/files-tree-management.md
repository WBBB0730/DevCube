...
文件树支持右键菜单，按「新建 → 打开 → 复制路径 → 重命名/删除」四组排布（危险项垫底）：所有目标可新建文件 / 新建文件夹（文件行作用于所在目录）；在文件夹中显示、在其他应用中打开（仅文件）、在终端中打开（文件开在所在目录）；复制路径 / 复制相对路径；非根条目可重命名 / 删除。新建与重命名走弹窗输入，删除弹确认并移入系统回收站（可恢复）。操作后树自动刷新，打开中的文件跟随重命名 / 删除联动。
...
14. As a 开发者, I want 在文件行上也能新建（作用于所在目录）, so that 不必先找到父目录行
15. As a 开发者, I want 复制路径 / 复制相对路径, so that 贴给终端或 AI 工具时不用手打
16. As a 开发者, I want 文件用系统默认应用打开, so that 图片 / PDF 这类只读预览能跳出去编辑
17. As a 开发者, I want 在某个目录直接开终端, so that 不用开完再 cd
18. As a 开发者, I want 在系统文件管理器中定位任意条目, so that 需要拖拽或精细操作时有出口
...
- 右键菜单复用 Base UI 受控 ContextMenu + 鼠标点虚拟 anchor（同 Git 图谱菜单模式，非行级 Trigger）；各目标可见项见 Solution 的四组排布（重命名 / 删除仅非根，其他应用打开仅文件，复制相对路径仅非根）。
- 弹窗走通用小对话框外壳（`ui/form-dialog`，抽取自 Git 对话框族、二者共用单一定义源；磁盘冲突弹窗一并收敛）：无标题栏、WebStorm 式提示语（「将 “x” 重命名为：」）+ 底部按钮条，Enter 提交带输入法合成保护，主按钮用动作动词（创建 / 重命名 / 删除）。确认回调抛错时弹窗保持打开、提交中锁死弹窗（含 Esc / 遮罩 / 取消）。
- 联动：重命名前先落未保存编辑；展开集 / 选中 / 最近列表按前缀重映射（shared 纯函数）；打开文件受影响时强制按新路径重开，再统一从磁盘刷新树（避免刷新把旧路径判「已消失」误清）。删除时撤掉挂起的自动保存计时器，防止把刚删的文件写回。
- 领域边界更新：CONTEXT.md 的 Files Tab 从「不做文件管理」放宽为支持新建 / 重命名 / 删除；复制 / 移动仍不做（复制路径是复制字符串，不是复制文件）。
- 「在终端中打开」给 Terminal 引入可选 cwd：IPC / 会话层透传，主进程限定项目内且存在的目录、越界或失效回落项目根；cwd 不随壳持久化（重启回项目根），CONTEXT.md 的 Terminal 词条同步放宽。文件行的新建与终端按「就近」语义作用于所在目录。
