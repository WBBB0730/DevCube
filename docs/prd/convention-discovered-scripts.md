...
- **文件监听**：在现有顶层 package/lockfile 之外，纳入第一批指纹相关路径（及 .NET 根级工程文件变化所需的最小监听）；仍不递归整树。
...
**好测试的标准。** 只测外部可观察行为：给定指纹事实 / 已晋升集合，得到的候补、晋升结果、对账删除、解析出的命令是否正确。不测 chokidar 本身、不测真实起进程。
