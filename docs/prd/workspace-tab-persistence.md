...
12. 作为开发者，我想只沿横向拖拽重排终端，并让重排后的顺序跨重启保留，以便操作方向与横向 Tab 栏一致且栏序稳定。
...
22. 作为开发者，我想 `Ctrl+Tab` 循环、CmdOrCtrl+W、CmdOrCtrl+T、CmdOrCtrl+1…9 直达 Tab 等快捷键在恢复后的栏上仍按现有规则工作，以便不用学新键位。
23. 作为开发者，我想 Tab 过多时把鼠标悬停在栏内直接用滚轮横向滚动，而不必按 Shift，以便快速找到被挤出视口的 Tab。
24. 作为开发者，我想每次激活 Tab（包括切换项目后恢复其激活 Tab）时，对应标签自动滚入横向视口，以便立刻看到当前所在位置。
...
- **Tab 栏溢出**：所有 Tab 保持单行、不收缩；只允许横向滚动，滚动条隐藏。左侧无留白，右侧固定保留 8px；左右 16px 渐变遮罩在对应方向还有超过 1px 的隐藏内容时完整显示，抵达边缘时隐藏，并带短透明度过渡且不拦截指针事件。栏内普通纵向滚轮映射为横向位移，触控板已有横向意图时沿用横向位移；无需 Shift。激活键或当前项目变化后，以最小距离把对应 Tab 滚入横向视口，并用与遮罩同宽的滚动内边距避免目标被遮挡。
